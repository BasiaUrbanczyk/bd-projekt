...
<?php
 ...
 $temp = pg_query($link, "select * from magazynowanie WHERE idmagazynowania = $id");
    $row = pg_fetch_array($temp, 0);
    $idEksponatu = $row["ideksponatu"];
    $wynik = pg_query($link, "DELETE FROM magazynowanie WHERE idmagazynowania = $id");


    $_SESSION['magazynowanieUsuniete'] = '<h1 class="error-text">Magazynowanie zostało usunięte</h1>';
    header('Location: historiaMagazynowanie.php?id='.$idEksponatu);
